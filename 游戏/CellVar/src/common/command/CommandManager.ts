...
module touch {
	 ...
	 * 命令管理器
	 */
	export class CommandManager extends egret.EventDispatcher implements IObjectable {
		/**
		 * 命令集合
		 */
		...
		private runCommand(): void {
			if (this._commands.length > 0) {
				...
				command.once(egret.Event.COMPLETE,this.onCommandRunOver,this)
				console.log("command run: " + command.name + " time " + egret.getTimer())
				command.run();
			}
			else {
