...
module touch {
	...
	export class MVCControlManager implements IDestoryable {
		 ...
		 * 控制器对象映射表
		 */
		private _controlMap: touch.Dictionary = null;

		/**
		 * 构造函数
		 */
		public constructor(main: egret.EventDispatcher, dataMgr: MVCDataManager, viewMgr: MVCViewManager) {
			//MVC主对象
			this._main = main;
			//数据管理器
			...
			//控制器对象映射表
			this._controlMap = new touch.Dictionary();
		}
